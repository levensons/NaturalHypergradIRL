...
class TTSANHD(nn.Module):
    def __init__(self, policy: PolicyNet, reward: RewardNet, n_cg_steps: int = 10, reg: float = 1e-3):
        super().__init__()
        self.policy = policy
        self.reward = reward
...
def main():
    ...
    report_path = Path(log_cfg.get("report_dir", "reports")) / "ttsa_cartpole_history.json"

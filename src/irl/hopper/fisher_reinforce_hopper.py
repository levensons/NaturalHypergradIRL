...
from src.utils.config import load_config, resolve_config_path, set_seed
from utils.logging import get_logger, save_history
...
class InnerOptimizer:
    ...
    def optimize(self, env, n_steps: int, n_traj: int):
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, n_steps, eta_min=self.eta_min)
        ...
        for _ in pbar:
            ...
            pbar.set_postfix({"g_raw": f"{stats['inner_grad_raw']:.2e}", "g_clip": f"{stats['inner_grad_clip']:.2e}"})


class OuterOptimizer:
    def __init__(self, reward: Reward, policy, lr: float, max_grad_norm, fisher_reg: float = 1e-2, gamma: float = 0.99):
        self.reward = reward
        self.policy = policy
...
def train_bilevel(env, expert_trajs, config: dict, logger=None) -> dict:
    ...
    for outer_step in range(1, n_outer_steps + 1):
        policy = Policy(state_dim=state_dim, action_dim=action_dim, hidden=hidden, n_hidden_layers=n_layers)
...
def main():
    ...
    report_path = Path(log_cfg.get("report_dir", "reports")) / "fisher_reinforce_hopper_history.json"

...
from utils.torch import flat_grad, assign_flat_gradients
...
class TTSANHD(nn.Module):
    def __init__(self, policy: GaussianPolicyNet, reward: RewardNet, n_cg_steps: int = 10, reg: float = 1e-3):
        super().__init__()
        self.policy = policy
        self.reward = reward
...
def train_ttsa(env, expert_trajs, config: dict, logger=None) -> dict:
    ...
    n_traj_per_step = int(n_traj_per_step)
    alpha_inner = float(alpha_inner)
    beta_outer = float(beta_outer)
    gamma = float(gamma)
    metrics_every = int(1)
    ...
    for k in range(1, n_iterations + 1):
        ...
        hg_norm_before_clip = hypgrad.norm().item() if torch.isfinite(hypgrad).all() else float("inf")
...
def main():
    ...
    report_path = Path(log_cfg.get("report_dir", "reports")) / "ttsa_hopper_history.json"
